<template>
	<view class="content">
		<view class="container">
			...
			<scroll-view class="scroll-view_H b-t b-b" scroll-x>
				<block v-for="(item,index) in dateArr" :key="index">
					...
					<div class="flex-box" @click="selectDateEvent(index,item)"
						:style="{'border-bottom':index==dateActive ? '2px solid  #2291f8' : ''}">
			...
			<!-- 时间选项 -->
			<view class="time-box" style="margin-bottom: 80rpx;">
				<block v-for="(item,_index) in timeArr" :key="_index">
					...
					<view class="item">
						<view class="item-box" :class="{'disable': item.disable ,'active':_index==timeActive}"
							...
							<text>{{item.time}}</text>
							<text class="all">剩余:{{item.num}}</text>
...
<script>
	...
	export default {
		...
		methods: {
			...
			setOnload() {
				this.dateArr = dateData(this.appointStartTime, this.appointEndTime) // 日期栏初始化
				this.timeArr = timeData(this.startTime, this.endTime, this.intervalTime / 60, this.appointPosNum) //时间选项初始化
				this.selectDate = `${this.dateArr[this.dateActive]['date']}` //默认选中的日期	
				this.currentTime = timeStamp(Date.now()).hour //当前时分秒
				let ifFullTime = true
				this.timeArr.forEach((item, index) => {
					if (this.timeQuery == this.nowdata && this.appointStartTime == 1) { //判断是当前这一天
						let itemTime = item.time.split(":");
						let itemAdd = parseInt(itemTime[0] * 60) + parseInt(itemTime[1])
						let itemAdd1 = itemAdd - this.intervalTime / 2
						let itemAdd2 = itemAdd + this.intervalTime / 2
						let currentTime1 = this.currentTime.split(":");
						let currentTime2 = parseInt(currentTime1[0] * 60) + parseInt(currentTime1[1])
						if (itemAdd1 <= currentTime2) { //选中时间小于当前时间则禁用
							item.disable = 1
						}
					}
					// 将预约的时间禁用
					this.appoTime.forEach(items => {
						...
						if (items.date == this.selectDate) {
							let itemsTime = items.time_id.split(":");
							let itemsAdd = parseInt(itemsTime[0] * 60) + parseInt(itemsTime[1])
							let itemTime = item.time.split(":");
							let itemAdd = parseInt(itemTime[0] * 60) + parseInt(itemTime[1])
							let itemAdd1 = itemAdd - this.intervalTime / 2
							let itemAdd2 = itemAdd + this.intervalTime / 2
							if (itemAdd1 <= itemsAdd && itemsAdd < itemAdd2) {
								item.num = item.num - items.used_pos_num
