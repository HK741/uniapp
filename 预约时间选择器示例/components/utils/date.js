...
//获取日期和礼拜天数
export function dateData(num1,num2) {
	const time = []
	const date = new Date()
	const now = date.getTime() //获取当前日期的时间戳
	let timeStr = 3600 * 24 * 1000 //一天的时间戳
	for (let i = num1-1; i < num1+num2; i++) {	
		const timeObj = {}
...
//时间数组
export function timeData(startTime, endTime, timeInterval,appointPosNum) {
	const time = []
